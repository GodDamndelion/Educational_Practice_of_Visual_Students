...
struct DoublyLinkedList
{
	...
	void Splice(ptrNODE_L<TInfo> pNode, DoublyLinkedList& list2);
	~DoublyLinkedList();
};
...
template<class TInfo>
inline void DoublyLinkedList<TInfo>::Splice(ptrNODE_L<TInfo> pNode, DoublyLinkedList& list2)
{
	ptrNODE_L<TInfo> p = list2.begin;
	while (p)
	{
		Add_after_node(pNode, p->info);
		p = p->next;
		if (!pNode)
		{
			pNode = begin;
